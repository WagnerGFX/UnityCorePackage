...
namespace CorePackage.StateMachine.Editor
{
    ...
    internal class TransitionTableEditor : UnityEditor.Editor
    {
            ...
            for (int i = 0; i < _fromStates.Count; i++)
            {
                ...
                Rect headerRect = BeginHorizontal();
                {
                    ...
                    // State Header Buttons
                    {
                        ...
                        Rect buttonRect = new(x: headerRect.width - 25, y: headerRect.y, width: 35, height: 20);
        ...
        {
            return _transitionsByFromStates[_fromStates.IndexOf(state)].ConvertAll(t => t.SerializedTransition);
        }
        ...
        {
            Dictionary<Object, List<TransitionDisplayHelper>> groupedTransitions = new();
            int count = _transitions.arraySize;
            for (int i = 0; i < count; i++)
            {
                SerializedTransition serializedTransition = new(_transitions, i);
